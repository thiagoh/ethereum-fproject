...
new Promise(function(resolve, reject) {
  ...
  var countAndCall = function(whenReaches, callback) {
    var counter = 0;
    var array = [];

    return function caller(index, data) {
      counter++;
      array[index] = data;
      if (counter >= whenReaches) {
        callback(array);
      }
    };
  };

  var countAndCaller = countAndCall(2, function(data) {
    console.log('both read');
    console.log(data);
    resolve(data);
  });

  fs.readFile('./bin/Conference.bin', function(err, data) {
    if (err) { console.error(err); throw err; }
    // console.log(data.toString());
    countAndCaller(0, data.toString());
  });

  fs.readFile('./bin/Conference.abi', function(err, data) {
    if (err) { console.error(err); throw err; }
    // console.log(data.toString());
    countAndCaller(1, data.toString());
  });
});
...
// console.log(argv);
var contractAddressFile = './bin/Conference.address';

new Promise(function(resolve, reject) {

    fs.readFile(contractAddressFile, function(err, data) {
      if (err) {
        console.error(err);
        reject(err);
      }
  ...
  .then(function(previouslyDeployedAddress) {
    ...
    if (argv.submit === true || argv.submit === 'true') {
          ...
          function(e, contract) {
            ...
            if (!e) {
              ...
              if (typeof contract.address !== 'undefined') {
                ...
                var write = function() {
                  fs.writeFile(contractAddressFile, contract.address);
                  resolve(contract.address);
                };

                fs.exists(contractAddressFile, function(exist) {
                  if (exist) {
                    fs.unlink(contractAddressFile, function() {
                      write();
                    });
                  } else {
                    write();
      ...
      return previouslyDeployedAddress;
    }
